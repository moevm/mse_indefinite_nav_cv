...
class StopLineFilterNode(DTROS):
    ...
    def get_stop_line_left_point(self, segment_list_msg):
        left_point_x = 10000.0
        left_point_y = 10000.0
        for segment in segment_list_msg.segments:
            if segment.color != segment.RED:
                continue
            if segment.points[0].x < 0 or segment.points[1].x < 0:  # the point is behind us
            ...
            if left_point_x > segment.points[0][0]:
                left_point_x = segment.points[0][0]
            if left_point_x > segment.points[1][0]:
                left_point_x = segment.points[1][0]
            if left_point_y > segment.points[0][1]:
                left_point_y = segment.points[0][1]
            if left_point_y > segment.points[1][1]:
                left_point_y = segment.points[1][1]
